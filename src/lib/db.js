...
db.version(6).stores({
  ...
  almacen_articulos: 'id, almacen_id, sku, updated_at',
  almacen_configuracion: 'almacen_id, updated_at',
  almacen_notificacion_emails: 'id, almacen_id, email, updated_at',
  sync_metadata: 'key',
  led_mappings: 'id, almacen_id, esp32Ip, channel, id_balda'
});
...
export async function replaceRawConfig({ modules, shelves, articles, settings = [], notificationEmails = [] }) {
  await db.transaction('rw', db.almacen_modulos, db.almacen_estantes, db.almacen_articulos, db.almacen_configuracion, db.almacen_notificacion_emails, async () => {
...
export async function mergeRawConfig({ modules, shelves, articles, settings = [], notificationEmails = [] }) {
  await db.transaction('rw', db.almacen_modulos, db.almacen_estantes, db.almacen_articulos, db.almacen_configuracion, db.almacen_notificacion_emails, async () => {
    if (modules.length) await db.almacen_modulos.bulkPut(modules);
    if (shelves.length) await db.almacen_estantes.bulkPut(shelves);
    if (articles.length) await db.almacen_articulos.bulkPut(articles);
